...
namespace EMelsec
{
    public class MelsecEthernet : IronInterface.Protocol.IProtocolBase
    {
        ...
        public bool InitializeProtocol(params object[] values)
        {
            ...
            {
                IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Melsec] : " + except.ToString());
                return false;
            }
            return true;
        }
       ...
       // public byte[] SetBlock(byte[] data, string devicecode, int command, bool isread)
        public byte[] SetBlockProtocol(byte[] data, params object[] values)
        {
            ...
            **/
            ...
            bool isbit = Utility.CheckIsBit(devicecode);
            byte[] b_subHeader;
            byte[] b_Qheader;
            byte[] b_command;
            byte[] b_subcommand;
        ...
        public string CheckBlockErrorProtocol(byte[] block)
        {
            try
            {
                ...
                if (dev.Binary)
                {
                    ...
                    if (errorcode != 0)
                    {
                        IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Melsec] : Error >>>>>>>>>> " + ErrorCodeCheck(errorcode));
            ...
            //IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Melsec] : Read Block No Error");
...
//        public object[] GetData(byte[] block, string deviceCode, int dataCount)
        ...
        byte[] GetCommand(int command)
        {
            ...
            if (command_code == 0)
            {
                IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Melsec] : CheckCommand Error coomand_code == 0");
                return null;
            }
        ...
        byte[] GetQHeader(byte[] data)
        {
            ...
            if (dev.Binary)
            {
                ...
                //IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Driver] : datalength : " + datalength);
        ...
        bool CheckIsBit(int devicecode)
        {
            ...
            switch (devicecode)
            {
                ...
                case 0xD0:
                    return false;
                default:
                    IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Melsec] : GetDevicecode Code Error return null >>>>> " + devicecode);
                    return false;
        ...
        byte[] ConvertToASCII(int data, int size)
        {
            ...
            {
                IronLog.LogManager.Manager.WriteLog("Exception_Driver",except.ToString());
                return null;
            }
        }
        ...
        ushort ConvertToHexushort(byte data)
        {
            ...
            { 
                ...
                case 0x46:
                    return 0xF;
                default:
                    IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Melsec] : ConvertToHexushort Error return 0 >>>>> " + data);
        ...
        byte[] ConvertToASCIIDigitalArrayWrite(ushort[] data)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                {
                    IronLog.LogManager.Manager.WriteLog("Exception_Driver","[EMelsec : Melsec] ConverToASCIIDigitalArrayWrite : " + except.ToString());
                }
            }
