...
namespace ModbusTCP
{
    ...
    public class Master
    {
        ...
        public void ReadCoils(ushort id, byte unit, ushort startAddress, ushort numInputs, ref byte[] values)
        {
            ...
            byte[] temp = CreateReadHeader(id, unit, startAddress, numInputs, fctReadCoil);
            values = WriteSyncData(temp, id);
        }
        ...
        public void ReadHoldingRegister(ushort id, byte unit, ushort startAddress, ushort numInputs, ref byte[] values)
        {
            ...
            byte[] temp = CreateReadHeader(id, unit, startAddress, numInputs, fctReadHoldingRegister);
            values = WriteSyncData(temp, id);
        }
        ...
        public void ReadInputRegister(ushort id, byte unit, ushort startAddress, ushort numInputs, ref byte[] values)
        {
            ...
            byte[] temp = CreateReadHeader(id, unit, startAddress, numInputs, fctReadInputRegister);
            values = WriteSyncData(temp, id);
        }
        ...
        private byte[] WriteSyncData(byte[] write_data, ushort id)
        {
            ...
            if (tcpSynCl.Connected)
            {
                try
                {
                    ...
                    TEMP[0] = tcpSynClBuffer[1];
                    TEMP[1] = tcpSynClBuffer[0];
                    short returnID = BitConverter.ToInt16(TEMP, 0);

                    if (transactionID != returnID)
                    {
                        int timeOri = tcpSynCl.ReceiveTimeout;
                        tcpSynCl.ReceiveTimeout = 100;
                        tcpSynCl.Receive(tcpSynClBuffer, 0, tcpSynClBuffer.Length, SocketFlags.None);
                        tcpSynCl.ReceiveTimeout = timeOri;

                        DriverLog.LogManager.Manager.WriteLog("Exception_Driver","[Modbus : Driver] : transactionID ERROR! (" + transactionID.ToString() + "/" + returnID.ToString() + ")");
                    }*/
                    
                    //DriverLog.LogManager.Manager.WriteLog("Exception", "[Modbus : Driver] : transactionID (" + transactionID.ToString() + "/" + returnID.ToString() + ")");
